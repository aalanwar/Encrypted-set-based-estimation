...
vector_t<double> Paillier_priv::decryptVector_f(const vector_t<encnum> &enc_v) const {
  vector_t<double> res (enc_v.rows());
  for(int i = 0; i < enc_v.rows(); i++) {
    res(i) = decrypt_f(enc_v(i));
  }
  return res;
}

vector_t<encnum> Paillier::addVector_f(const vector_t<encnum> &a, const vector_t<encnum> &b) const {
  assert(a.rows()==b.rows() && "Both vectors need to have the same dimension!");
  vector_t<encnum> res (a.rows());
...
vector_t<encnum> Paillier::subVector_f(const vector_t<encnum> &a, const vector_t<encnum> &b) const {
  assert(a.rows()==b.rows() && "Both vectors need to have the same dimension!");
  vector_t<encnum> res (a.rows());
  for(int i = 0; i < res.rows(); i++){
    res(i) = sub_f(a(i),b(i));
...
vector_t<encnum> Paillier::multConstMatrixVector_f(const matrix_t<double> &m, const vector_t<encnum> &v){
  assert(m.cols()==v.rows() && "Columns of matrix and rows of vector have to match!");
  vector_t<encnum> res (m.rows());
  for(int i = 0; i < m.rows(); i++){
    encnum rowres = constMult_f(m(i,0),v(0));
    for(int j = 1; j < m.cols(); j++){
      rowres = add_f(rowres, constMult_f(m(i,j),v(j)));
    }
    res(i) = rowres;
  }
  return res;
...
vector_t<encnum> Paillier::multConstVector_f(const double &s, const vector_t<encnum> &v){
  vector_t<double> d = vector_t<double>::Constant(v.rows(), s);
  matrix_t<double> diag = d.asDiagonal();
  return multConstMatrixVector_f(diag, v);
}
