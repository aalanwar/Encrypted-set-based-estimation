...
namespace EncEst 
{
	...
	{
		// SJ: TODO
		// Check::Eq(mp, ez.mP, Msg::PublicKeyMissmatch);
		// Check::Eq(mDimension, ez.mDimension, Msg::DimensionMissmatch);
		// Check::Eq(mp, ez.mp, "Override default message");

		// SJ: Potential Type because we are comparing the adresses of the Paillier pointer which indeed can be different without
		// SJ: The pubkey() beeing different.
		// if(&mP != &(ez.mP))
	...
	std::vector<matrix_t<double>> EncZonotope::computeLambdas(const std::vector<EncStrip>& encstrips) 
	{
		...
		for(int i=1; i< encstrips.size(); i++)
		{
			...
			tmph << h_combined, encstrips[i].mH; 
			h_combined = tmph;				  
		}
