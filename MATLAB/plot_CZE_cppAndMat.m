...
load('CMatFiles/cppCZonoEntities.mat')
%get the number of var in workspace
variablesInCurrentWorkspace = who;
numVariables = length(variablesInCurrentWorkspace);
index =1;
numofiteration = floor(numVariables/4)-1;
for i=0:numofiteration
    c_i= eval(strcat('c_',num2str(i)));
    G_i= eval(strcat('G_',num2str(i)));
    A_i= eval(strcat('A_',num2str(i)));
    b_i= eval(strcat('b_',num2str(i)));
    czonol{index} = conZonotope([c_i,G_i],A_i,b_i);
    sup(index,:) = supremum(interval( czonol{index}));
    infi(index,:) = infimum(interval( czonol{index}));
    cen(index,:) = czonol{index}.center;
    index = index +1;
end

save('CMatFiles/cppsummary-CZE.mat', 'czonol', 'sup', 'infi', 'cen','numofiteration');

load('cache/paper/matCZE.mat');
%load('cache/temp.mat');
